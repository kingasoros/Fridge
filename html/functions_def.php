...
function registerUser(PDO $pdo, string $password, string $firstname, string $lastname, string $email, string $token): int
{
    ...
    $sql = "INSERT INTO profil(password,first_name,last_name,email,registration_token, registration_expires,active)
...
function sendEmail(PDO $pdo, string $email, array $emailData, string $body, int $id_user): void
{
    ...
    try {
        ...
        $phpmailer->Username = 'f3c7fed18c1a3b';
        $phpmailer->Password = 'rerre';
        $phpmailer->setFrom('kingasoros@gmail.com', 'Webmaster');
...
{
    $sql = "UPDATE profil SET forgotten_password_token = :token, forgotten_password_expires = DATE_ADD(now(),INTERVAL 6 HOUR) WHERE email = :email";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':token', $token, PDO::PARAM_STR);
    $stmt->bindParam(':email', $email, PDO::PARAM_STR);
    $stmt->execute();
